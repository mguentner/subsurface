			...
			if (state == FINDSTART) {
				...
				if (i > 1) {
					if(buffer[i-1] != '\\') {
						...
						tag[end-start+1] = '\0';
						state=FINDSTART;
						/* Add tag to global tag list, add the returned
						 * divetag to the dive's taglist
						 */
						divetag = taglist_add_tag(g_tag_list, tag);
						taglist_add_divetag(tags, divetag);
					}
				} else {
					state=FINDSTART;
				}
