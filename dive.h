...
struct divetag {
	char *name;
	/*
	 * We provide a translation for default tags and write
	 * the source text to the savefile
	 */
	char *source;
};
 ...
 * a list of tag_entrys pointing divetags
 */

extern struct tag_entry *g_tag_list;

struct divetag *taglist_add_divetag(struct tag_entry *tag_list, struct divetag *tag);
struct divetag *taglist_add_tag(struct tag_entry *tag_list, const char *tag);

/*
 * Writes all divetags in *tag_list to *buffer, limited by the buffer's len).
